...
// For example, in the expression /\s+/g, the / symbol marks the start and end of the pattern \s+. The \s is a shorthand character class that matches any white space character (e.g. spaces, tabs, and line breaks), and the + means that one or more of these characters can appear together. The g at the end of the expression is a global flag, which means that the pattern will be searched for globally in the string, and not just the first occurrence.
  ...
  <textarea className="form-control" placeholder="Enter text here" id="floatingTextarea2" style={{backgroundColor:colour==="light"?"#D8D8D8":"white"}} value={content} onChange={change}></textarea>
  <label for="floatingTextarea2"></label>
  <button type="button" className={`btn btn-outline-${colour} mx-1`} onClick={upperCase}>Change to Uppercase</button>
  <button type="button" className={`btn btn-outline-${colour} mx-1`} onClick={clear}>Clear text</button>
  <button type="button" className={`btn btn-outline-${colour} mx-1`} onClick={copy}>Copy text</button>
  <button type="button" className={`btn btn-outline-${colour} mx-1`} onClick={spaces}>Remove extra spaces</button>
<h1 className={`my-2`}>Your text summary</h1>
<span>{
  (content==="" ||content.trim()==="")?0:content.split(/\s+\w/).length} words and {content.length} characters</span>
  {/* if space is not followed by a word,its not counted */}
{(content!=="" || content.trim()!=="") && <div>{ (0.008*content.split(/\s+/g).length).toFixed(4)} minutes read</div>}
</div>
</>
  )
}
